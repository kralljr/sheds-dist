...
1. Scale up real datai from ZCTA to all of Atlanta
2. Perform simulation study using real exposure distribution data
...
In order to perform the simulation study, we first had to generate distributions of exposures for each day.  To start, we assumed that the distribution of exposures on each day was truncN$(0.5, 4^2)$ where the concentrations were truncated below by zero.  The histogram of this distribution is shown below.  Then, we modified this distribution according to how the distributions might change day-to-day, as described in the following sections. 




```{r truncn}
x <- rtruncnorm(10000, a = 0, mean =0.5, sd = 4)
...
```{r beta, fig.height = 20, fig.width = 15}
ag1 <- c(0.1, 0.25, 0.5, 0.75, 0.9)

tx <- c("shift", "longr", "longl", "wide")
nx <- c("Shift", "Long right", "Long left", "Wide")
...
for(xi in 1 : 4) {
  ...
  x1 <- getx(ny, naG, argvals2, typex = tx[xi], ns1 = ns2)
  ...
  for(bi in 1 : 6) {
	...
	tb2 <- qbs[[bi - 4]]
      ...
      sim1 <- simout(x1, argvals2, argvalslr = ag1, typeb = tb2, 
        sd1 = sd2, disttype = disttype1)
    #}
