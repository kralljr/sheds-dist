...
nx <- c("Shift", "Long right", "Long left", "Wide", "True PM")
...
tb2b <- c("constant", "high", "low", "high", "low", "x2", "high", "low", "x2")
xi2a <- c("shift", "longr", "longl", "wide", "wide", "wide", "true", "true", "true")
xi2b <- c(1, 2, 3, 4, 4, 4, 5, 5, 5)


# Get true betas
for(i in 1 : length(tb2b)) {
	gb1 <- getbeta(tb2b[i])
	betas <- gb1(argvals2)

        tb2 <- tb2b[i]
        type1 <- rep(paste(tb2, ":", xi2a[i]), length(betas))
        data1 <- data.frame(argvals2, betas, type1)
...
for(sall in 1 : length(xi2b)) {
  ...
  sim1 <- simout(xuse1, argvals2, betaM = 0, argvalslr = ag1, typeb = tb2, 
    sd1 = sd2, disttype = disttype1, val1 = 0, quants = F, std = T)


   x <- as.numeric(rownames(sim1$beta2))
   type1 <- rep(paste(tb2, ":", xi2a[sall]), length(x))
...
ggplot() +xlab("Quantile") + ylab("Beta estimate") +
...
linfun <- 1 / length(ag1) * rowSums(sweep(xuse, 2, betas1, "*"))
set.seed(10)
